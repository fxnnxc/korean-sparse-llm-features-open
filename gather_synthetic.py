...
activations ={'synthetic_input_ids':[], 
...
def estimate_token_location(text, target, tokenizer, window_size=5):
    ...
    text_tokens = tokenizer.encode(text, return_tensors='pt', padding='max_length', max_length=128)[0]
...
for batch in pbar:
    ...
    for i in range(input_ids.shape[0]):
        ...
        activations[f'{name}_residual_q2'].append(dict_format['residual_q2']['module'].fetched_values[0][i].detach().cpu()[loc,:])
        activations[f'{name}_residual_q3'].append(dict_format['residual_q3']['module'].fetched_values[0][i].detach().cpu()[loc,:])
